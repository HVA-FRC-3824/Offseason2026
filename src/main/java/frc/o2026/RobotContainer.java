...
public class RobotContainer extends SubsystemBase {
  ...
  private static enum LimitingMode {
    TrigExp,
    Linear;

    @AutoLogOutput(key = "LimitingMode")
    @Getter
    @Setter
    static LimitingMode mode = LimitingMode.TrigExp;
  }
  ...
  private static enum ControlMode {
    ...
    public static boolean isMatch() {
      return mode == ControlMode.Match;
    }

    public static boolean isTesting() {
      return mode == ControlMode.Testing;
    }
  }

  @AutoLogOutput(key = "trim")
  private AngularVelocity m_trim = RPM.of(0.0);

  @AutoLogOutput(key = "manual")
  private AngularVelocity m_manual = RPM.of(0.0);
  ...
  public RobotContainer() {
    ...
    Supplier<Command> fireWhenReady =
        () ->
            Commands.parallel(
                ...
                m_intake.setState(IntakeDesiredState.stowed).repeatedly());
    ...
    Periodical.addPeriodic(
        2, () -> m_manual = RPM.of(SmartDashboard.getNumber("trim/speed", m_manual.in(RPM))));
    ...
    m_operator.povUp().onTrue(m_intake.setState(IntakeDesiredState.stowed));
    ...
    m_driver
        ...
        .onTrue(Util.runOnce(() -> m_manual = m_manual.plus(RPM.of(100.0))));

    m_driver
        .povUp()
        .and(ControlMode::isTesting)
        .onTrue(Util.runOnce(() -> m_manual = m_manual.minus(RPM.of(100.0))));

    m_driver
        .leftBumper()
        .and(ControlMode::isTesting)
        .whileTrue(
            m_swerve.setState(SwerveDesiredState.aimPass.with(this::getSpeeds)).repeatedly());

    m_driver
        .rightBumper()
        .and(ControlMode::isTesting)
        .whileTrue(
            m_swerve.setState(SwerveDesiredState.aimSOTM.with(this::getSpeeds)).repeatedly());
    ...
    m_driver
        .rightTrigger()
        .and(ControlMode::isTesting)
        .whileTrue(m_flywheel.setState(FlywheelDesiredState.manual.with(() -> m_manual)));
