...
public class PoseEstimator {
  ...
  /**
   * Resets the odometry to a known pose and simultaneously re-seeds the swerve module baseline so
   * the next {@link #addDriveData(double, SwerveModulePosition[], Rotation2d)} call produces
   * correct deltas.
   */
  ...
  /**
   * Records a new drive movement expressed as a pre-computed {@link Twist2d}. Use this when you are
   * not using the swerve-module-delta path.
   */
   ...
   *
   * <ul>
   *   <li>Each module's individual distance delta is used, so per-module slip is handled correctly
   *       by the kinematics inverse.
   *   <li>The heading component of the resulting twist is replaced with the gyro-measured rotation,
   *       which is far more accurate than the wheel-integrated heading.
   * </ul>
   *
   * <p>Requires the swerve-aware constructor to have been used, or {@link
   * #setKinematics(SwerveDriveKinematics, SwerveModulePosition[], Rotation2d)} to have been called.
   *
  ...
  /**
   * Shared logic for inserting a single {@link VisionUpdate} at a given timestamp. Handles both the
   * "slot already exists" and "interpolate a new slot" cases.
   */
  private void insertVisionUpdate(double timestamp, VisionUpdate visionUpdate) {
    if (updates.containsKey(timestamp)) {
      ...
      var updateTwist = nextUpdate.getValue().twist();
      var twist0 =
          new Twist2d(updateTwist.dx * alpha, updateTwist.dy * alpha, updateTwist.dtheta * alpha);
      var twist1 =
          new Twist2d(
              updateTwist.dx * (1.0 - alpha),
              updateTwist.dy * (1.0 - alpha),
              updateTwist.dtheta * (1.0 - alpha));
